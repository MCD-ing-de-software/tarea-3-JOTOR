...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        with self.assertRaises(KeyError, msg="Columna no existe"):
            cleaner.drop_invalid_rows(df=df, cols=["does_not_exist"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        df = make_sample_df()
        cleaner = DataCleaner()
        result = cleaner.trim_strings(df=df, cols=["name"])
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        with self.assertRaises(TypeError, msg="Columna es numerica"):
            cleaner.trim_strings(df=df, cols=["age"])
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        ################################################  Notas ######################################################
        # - El factor se reajustó a 0.5
        # Con los valores no nulos en el datframe (25, 35, 120) 
        # Q1 = 30
        # Q3 = 77.5
        # IQR = 47.5 (Q3 - Q1)
        # Lower = -41.25 [Q1 - (1.5 * IQR)]
        # Upper = 148.75 [Q3 + (1.5 * IQR)] -> Por lo tanto, con un factor de 1.5 no se retira el valor outlier de 120
        # Upper con 0.5 = 101.25 [Q3 + (0.5 * IQR)] -> Por lo tanto se retira el outlier de 120.
        ###############################################################################################################
        result = cleaner.remove_outliers_iqr(df=df, col="age", factor=0.5)
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError, msg="Columna no existe"):
            cleaner.remove_outliers_iqr(df=df, col="salary")
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        with self.assertRaises(TypeError, msg="Columna no es numérica"):
            cleaner.remove_outliers_iqr(df=df, col="city")
